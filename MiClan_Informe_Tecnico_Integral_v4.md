...
**Actualización 2026-08-11 (segunda vuelta, mismo día):** al probar el panel recién agregado, el usuario detectó dos cosas sospechosas ("siempre muestra 1 min aunque el miembro tenga 5 min configurado" y "última actualización dice 'ahora mismo', no estoy seguro que sea cierto") y pidió verificar si el botón "Desactivar optimización de batería" hace algo real. Las tres revisiones encontraron:
1. **Bug real y serio, no solo del panel** (sección 13.8): "Modo ahorro" no tenía NINGÚN efecto en el intervalo real de grabación desde el fix de la sesión anterior, por una mezcla de dos storages de `SharedPreferences` distintos e incompatibles entre sí en Android. Corregido.
2. **Hueco de diseño real en el panel** (sección 10.7): el "latido" del servicio podía verse fresco aunque el GPS no estuviera produciendo ubicaciones. Se agregó un dato separado ("Última ubicación GPS") con su propia lógica de estado.
3. **El botón de batería confirmado funcional**, con una limitación real e importante documentada (sección 10.8): solo cubre el sistema estándar de Android, no los gestores de batería propios de cada fabricante.
...
### 10.5 Bugs de GPS resueltos en la sesión del 2026-08-10 (root-caused con `adb logcat` en dispositivo real — ver sección 13.9 para la metodología completa)
...
**Refinamiento 2026-08-11 (mismo día, segunda vuelta) — separar "el proceso vive" de "el GPS funciona":** el usuario notó, con razón, que "Última actualización: ahora mismo" no le daba confianza total. El motivo real: el reporte de `deviceStatus` se hace **antes** de intentar leer el GPS (a propósito, ver más arriba), así que puede seguir fresco aunque `Geolocator.getCurrentPosition()` esté fallando en silencio. Se agregó `FirestoreService.getGroupLastLocationStream(groupId)` (stream de `locations/{uid}.timestamp` por miembro, vía `groupLastLocationProvider`) como una señal **separada y más honesta**: la fecha de la última ubicación **efectivamente grabada**, no solo "el servicio sigue tickeando". La tarjeta de cada miembro ahora muestra ambos datos por separado ("Última ubicación GPS" primero, "Latido del servicio" después), y el estado se recalcula con 4 casos en vez de 3:
- **Sin datos** (gris): nunca reportó nada.
- **Desconectado / sin señal reciente** (rojo): ni siquiera el latido del servicio es reciente — el proceso probablemente está muerto.
- **Servicio activo, SIN ubicación GPS reciente** (naranja, caso nuevo): el latido es reciente pero la última ubicación real no lo es — el proceso vive pero el GPS no está produciendo datos. **Este es probablemente el caso real que motivó el panel.**
- **App activa / Segundo plano** (verde/azul): ambos son recientes.

### 10.8 El botón "Desactivar optimización de batería" — qué hace realmente (verificado 2026-08-11)

`SettingsScreen._openBatterySettings()` lanza el intent nativo estándar de Android:
```dart
...
intent.launch();
```
Esto **es funcional, no un botón decorativo**: `ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` es la API pública de Android para mostrar directamente el diálogo nativo "¿Permitir que MiClan ignore las optimizaciones de batería?" (requiere el permiso `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`, ya declarado en el manifest). Si el usuario acepta, el SO agrega la app a la whitelist estándar de Doze/App Standby de inmediato — se puede confirmar leyendo `Permission.ignoreBatteryOptimizations.isGranted` (`BatteryOptimizationService.isIgnoring()`).

**Limitación real e importante, no resoluble por código:** esta API estándar de Android **solo controla el sistema de optimización de batería de AOSP puro**. **No toca los gestores de batería propios de cada fabricante** — Xiaomi/MIUI tiene su propio "Inicio automático" (Autostart), Huawei/Honor tiene "Apps protegidas", Samsung tiene su lista de "Apps en reposo" en modos de ahorro agresivo, Oppo/ColorOS y Vivo tienen equivalentes propios. Estos sistemas paralelos pueden seguir matando el proceso en segundo plano **aunque la app ya esté correctamente excluida de la optimización estándar** — hay que habilitarlos manualmente, por separado, en los ajustes específicos de esa marca (no hay una API pública estándar de Android para hacerlo desde la app). Motorola (el teléfono de prueba usado en esta sesión) corre una versión de Android bastante cercana a AOSP, así que ahí es más probable que el botón estándar alcance por sí solo; en Xiaomi/Huawei/Samsung/Oppo probablemente no.

**Mejora agregada 2026-08-11:** el tile del botón en Configuración ahora muestra el estado real en vivo (ícono y subtítulo cambian según `BatteryOptimizationService.isIgnoring()`), y se re-chequea automáticamente al volver de Ajustes (`WidgetsBindingObserver`, mismo patrón que ya usa `HomeScreen` para el mismo permiso) y también ~800ms después de tocar el botón (por si el diálogo del sistema se resuelve sin pasar por un `paused`/`resumed` real de la Activity). Antes no había ninguna forma de confirmar, sin salir a Ajustes del sistema, si el botón realmente había surtido efecto.
...
### 13.8 "Modo ahorro" no tenía ningún efecto real en el intervalo de grabación — encontrado 2026-08-11

**Síntoma que lo destapó:** en el panel "Estado de Miembros" (recién agregado, sección 10.7), el intervalo mostrado para un miembro decía siempre "1 min" aunque ese miembro tuviera "Modo ahorro" activado (que debería ser 5 min) en su propia Configuración.

**Causa real:** `SharedPreferencesAsync` (usada en `location_config.dart` desde el fix de la sección 13.7/10.1, para evitar el caché por isolate) **usa por defecto, en Android, el backend "DataStore" (Jetpack DataStore Preferences)** — un almacenamiento **completamente distinto** del archivo clásico que usa `SharedPreferences.getInstance()` (confirmado leyendo el código fuente de `shared_preferences_android`: `SharedPreferencesAsyncAndroidOptions({this.backend = SharedPreferencesAndroidBackendLibrary.DataStore, ...})`). `settings_screen.dart` seguía escribiendo/leyendo el switch "Modo ahorro" con el `SharedPreferences.getInstance()` clásico — un storage que `location_config.dart` nunca llegaba a leer. El switch se veía correcto en la propia pantalla de Configuración (autoconsistente: leía y escribía siempre del mismo storage clásico), pero el intervalo real usado por el timer de sync en primer plano, por el tick loop del servicio de background, y por el nuevo reporte de `deviceStatus`, **quedaba siempre en el valor por defecto (1 minuto), sin importar lo que el usuario configurara**. Esto estuvo así desde el fix de la sesión anterior (2026-08-10) sin que nadie lo notara, porque no había ningún indicador visible del intervalo real hasta que se agregó el panel de monitoreo.

**Nota aparte, mismo bug al revés:** `location_service.dart._handlePosition` (el throttle de distancia 50m/200m) leía `battery_saver` con el `SharedPreferences` clásico — coincidía por casualidad con lo que escribía Configuración (los dos usaban el storage clásico), así que ESE throttle sí funcionaba. Es decir, antes de este fix, "Modo ahorro" afectaba el umbral de distancia pero NO el intervalo de tiempo — una mezcla inconsistente y confusa de dos comportamientos distintos según qué parte del código se mirara.

**Fix:** se centralizó el flag `battery_saver` en `location_config.dart` (`isBatterySaverEnabled()` / `setBatterySaverEnabled()`, ambas sobre la misma instancia de `SharedPreferencesAsync`), y se migraron **todos** los puntos que lo leían o escribían (`settings_screen.dart` en ambos sentidos, `location_service.dart._handlePosition`) para usar exclusivamente estas funciones. Ya no debería volver a divergir porque hay un solo lugar que toca ese storage.

**Lección para cualquier flag nuevo que se comparta entre isolates:** `SharedPreferencesAsync` resuelve el problema de caché por isolate (sección 13.7), pero **no es intercambiable con el `SharedPreferences` clásico** aunque tengan una API parecida — son storages distintos en Android por defecto. Cualquier flag que se lea/escriba desde más de un lugar debe pasar por un único helper centralizado (como ahora hace `location_config.dart`), nunca leerse/escribirse directo con una u otra API indistintamente.

### 13.9 Metodología para 13.6/13.7 (aplicable a cualquier bug futuro "no cambia nada")
...
12. Antes de dar por buena cualquier corrección de bug reportado como "no cambia nada": conseguir un dispositivo real por `adb` y usar `adb logcat` con logs temporales en vez de seguir iterando solo por lectura de código (sección 13.9) — ahorra rondas enteras de hipótesis equivocadas.
